...
Simulates orders being published to Pub/Sub topic in real-time.
Useful for testing streaming pipelines locally.

Author: Dorra Trabelsi
Date: 2026-04-21

COST NOTE: Pub/Sub free tier = 10GB/month.
Each message is ~500 bytes. 200 messages = ~100KB → negligible cost.
Default --limit is 200. Never run with --limit 0 (unlimited) unless you
...
DATA_FILE = Path("data/clean/orders_clean.csv")


def main():
    """Publish simulated order events to Pub/Sub."""
    parser = argparse.ArgumentParser(
        description="Simulate real-time order stream to Pub/Sub"
    )
    parser.add_argument(
        "--limit",
        type=int,
    ...
    try:
        logger.info("=" * 80)
        logger.info("REAL-TIME ORDER STREAM SIMULATOR")
        logger.info("=" * 80)
        ...
        logger.info(f"Loaded {len(orders_df):,} orders from {DATA_FILE}")
        ...
        if error_count == 0:
            logger.info("✓ Stream simulation complete - no errors")
            return 0
        else:
            logger.warning(f"⚠ Stream simulation complete with {error_count} errors")
            return 1
